...
impl Raytracer {
    ...
    pub fn random_bounded(&mut self, min: f64, max: f64) -> Vec3 {
        Vec3(self.f64_random_bounded(min, max), self.f64_random_bounded(min, max), self.f64_random_bounded(min, max))
    }
        ...
        *depth += 1;
        ...
        match opt_hit_record {
            Some(hit_record) => {
                ...
                0.5 * self.ray_color(Ray { origin: hit_record.p, direction: target - hit_record.p }, hittable, depth)
            }
            None => {     
        ...
        for y in 0u32..self.image_height {
            ...
            for x in 0u32..self.image_width {
                ...
                for s in 0u32..self.samples_per_pixel {
                    let horizonal_offset = (x as f64 + self.f64_random()) / (self.image_width - 1) as f64;
                    let vertical_offset =  ((self.image_height - 1 - y) as f64 + self.f64_random()) / (self.image_height - 1) as f64;
